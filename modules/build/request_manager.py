...
class Request:
    ...
    Responsibilities:
      - Store immutable user intent (dataset, pipeline_sequence, stage_args, parameters)
      - Maintain provenance (user, host, timestamp, pipeline version)
      - Manage job lineage (list of job_ids)
      - Create and chain jobs using canonical stage_input → stage_output logic
      - Support continuation from a previous request/job
    """
    ...
    def create_new(cls, base_dir, dataset, pipeline_spec, parameters=None):
        ...
        request.update_pipeline_state(state="running")
    ...
    # ------------------------------------------------------------
    # Preferred index-based sequencing (modern)
    # ------------------------------------------------------------
    def create_next_job_by_index(self, current_index, extra_parameters=None):
        ...
        # Determine previous stage output
        stage_input = None
        if 0 <= current_index < len(self.jobs):
            ...
            if prev_stage in Job.STAGE_OUTPUTS:
                output_filename = Job.STAGE_OUTPUTS[prev_stage]
                prev_outputs = os.path.join(self.jobs_dir, prev_job_id, "outputs")
                stage_input = os.path.join(prev_outputs, output_filename)
        ...
        if stage_input is not None:
            params["stage_input"] = stage_input
    ...
    def update_pipeline_state(self, **kwargs):
        """
        Update or create pipeline_state.json with the latest pipeline status.
        """
        state_path = os.path.join(self.request_dir, "pipeline_state.json")
